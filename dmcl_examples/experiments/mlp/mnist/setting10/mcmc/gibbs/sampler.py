...
from dmcl_examples.experiments.mlp.mnist.setting10.mcmc.dataloaders import training_dataloader
from dmcl_examples.experiments.mlp.mnist.setting10.model import model

# %% Setup Gibbs sampler

scales = [np.sqrt(0.005) for _ in range(10)] + \
    [np.sqrt(0.00005) for _ in range(10)] + \
    [np.sqrt(0.00005) for _ in range(10)] + \
    [np.sqrt(0.000005) for _ in range(10)]
