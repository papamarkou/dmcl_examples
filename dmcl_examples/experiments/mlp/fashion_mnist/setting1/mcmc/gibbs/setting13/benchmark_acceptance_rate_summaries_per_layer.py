...
from dmcl_examples.experiments.mlp.fashion_mnist.setting1.mcmc.gibbs.setting13.constants import summary_output_run_paths
...
for i in range(num_chains):
    acceptance_rates.append(np.loadtxt(summary_output_run_paths[i].joinpath('acceptance_rates.txt')))
...
for i in range(num_chains):
    np.savetxt(summary_output_run_paths[i].joinpath('acceptance_rate_means_per_layer.txt'), means[:, i])
    np.savetxt(summary_output_run_paths[i].joinpath('acceptance_rate_medians_per_layer.txt'), medians[:, i])
